...
The branding image in `../branding/` is used as this video's poster frame, so the
hero paints instantly and holds a still under `prefers-reduced-motion`.

Build step converts the source to `hero.webm` + `hero.mp4` + `hero-poster.jpg`.
